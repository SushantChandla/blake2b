...
class Blake2b {
  ...
  List<BigInt> _internalState = List<BigInt>.filled(
      16, BigInt.from(0)); // In the Blake2b paper it is called: v

  List<BigInt>?
      _chainValue; // state vector, in the Blake2b paper it is called: h

  BigInt _t0 =
      BigInt.from(0); // holds last significant bits, counter (counts bytes)
  BigInt _t1 = BigInt.from(0); // counter: Length up to 2^128 are supported
  BigInt _f0 = BigInt.from(0); // finalization flag, for last block: ~0L

  void init() {
    if (null == _chainValue) {
      final List<BigInt> newChainValue = [];

      // 0
      newChainValue.add(Const.blake2bIv[0] ^
          BigInt.from((_digestSize | (_keyLength << 8) | 0x1010000)));
  ...
  void update0(final int b) {
    ...
    if (remainingLength == 0) {
      _t0 += BigInt.from(Const.blockLengthBytes);
      if (_t0 == BigInt.from(0)) {
        _t1 += BigInt.from(1);
      }
      _compress(_buffer, 0);
      _buffer.fillRange(0, _buffer.length, 0);
  ...
  Uint8List digest(Uint8List out, final int outOffset) {
    _f0 = Const.f0;
    _t0 += BigInt.from(_bufferPos);

    if (0 < _bufferPos && _t0 ==BigInt.from(0)) {
      _t1+=BigInt.from(1);
    }
    ...
    _buffer.fillRange(0, _buffer.length, 0);
    _internalState =
        List<BigInt>.filled(_internalState.length,BigInt.from(0));
    ...
    _chainValue = List<BigInt>.filled(_chainValue!.length, BigInt.from(0));
  ...
  void _compress(Uint8List message, int messagePos) {
    ...
    List<BigInt> m = List<BigInt>.filled(16, BigInt.from(0));
    for (int j = 0; j < 16; j++) {
  ...
  void _g(BigInt m1, BigInt m2, int posA, int posB, int posC, int posD) {
  ...
  List<BigInt> _arrayCopyI64(List<BigInt>? src, int srcOffset, List<BigInt> dst,
